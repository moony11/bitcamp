...
public class Ex01_Hello extends HttpServlet {
    ...
    public Ex01_Hello() {
    	/* 생성자
    	 WAS(Web Application Server : 웹어플리케이션 서버 - Servlet 컨테이너,
    	 	JSP 컨테이너) 에서
    	 서블릿이 최초 등록될때(최초요청시) 한번만 실행
    	 한번 로딩이 되면 재실행되지 앟ㄴ는다.
    	 두번째 URL요청부터는 service() 메소드 실행됨
    	 */
	...
	public void destroy() {
		// 서버에서 서블릿이 내려갈때(소멸될때) destroy() 호출되고 삭제됨
