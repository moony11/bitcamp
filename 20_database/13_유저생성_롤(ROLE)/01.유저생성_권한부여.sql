...
----- (SYSTEM) 권한부여 - MADANG.BOOK 을 SELECT  할 수 있는 권한을 MDGUEST에게부여 
SELECT * FROM MADANG.BOOK; -- 권한받기 전 : MDGUEST 유저 조회 못함
--(SYSTEM) 권한부여 
...
-- (MDGUEST) SELECT, UPDATE 권한 취소후 테이블에 접근할 수 없음
...
-- WHIT GRANT OPTION : 다른유저에게 권한 부여 할 수 있도록 허용
-- (MADANG)MADANG 유저가 MDGUEST 에게 권한 부여

GRANT SELECT, UPDATE ON MADANG.CUSTOMER TO MDGUEST WITH GRANT OPTION;-- 권한부여
-- (MDGUEST) MDGUEST 유저가 MDGUEST2에게 권한부여 가능 
GRANT SELECT, UPDATE ON MADANG.CUSTOMER TO MDGUEST2;
-- (MDGUEST2) MDGUEST2 유저가 CUSTOMER 테이블 SELECT, UPDATE 가능 
-------
--(MADANG) MADANG 유저가 MDGUEST로부터 권한회수(취소)
REVOKE SELECT, UPDATE ON CUSTOMER FROM MDGUEST; -- 권한 회수
-- MDGUEST에게 부여된 CUSTOMER 테이블 SELECT, UPDATE 권한이 취소됨
-- MDGUEST가 MDGUEST2에게 부여한 CUSTOMER 테이블 SELECT, UPDATE 권한 또한 취소됨
