/* 프로시저(PROCEDURE)*/
SET SERVEROUTPUT ON; -- 서버에 출력정보 표시 설정
...
BEGIN -- 실행문시작
    ...
    DBMS_OUTPUT.PUT_LINE(V_EMPID || ' : ' || V_NAME);  -- SYSOUT 이랑 같은 개념
END; -- 실행문 끝
...
- OUT : 출력만 하는 변수
    (값을 받을 수 없고, 프로시저가 실행된 후 호출한 곳으로 변수를 통해 값을 전달)
...
CREATE OR REPLACE PROCEDURE GET_BOOKINFO (
    ...
    V_BOOKID BOOK.BOOKID%TYPE; -- BOOK 테이블의 BOOKID TYPE을 그대로 쓰겠다 
...
BEGIN
  ...
  DBMS_OUTPUT.PUT_LINE('>> TEST ' || ', ' || IN_CUSTID || ', ' || V_NAME || ', '
             || V_ADDRESS || ', '|| V_PHONE);
...
BEGIN
  DBMS_OUTPUT.PUT_LINE('>>> TEST IN CUSTID : ' || IN_CUSTID);
    DISP_CUSTINFO(IN_CUSTID, V_NAME, V_ADDRESS, V_PHONE);
    DBMS_OUTPUT.PUT_LINE('>>>TEST OUT값 - ' || V_NAME || '/' ||
                V_ADDRESS || '/' || V_PHONE);
END DISP_CUSTINFO_TEST;
