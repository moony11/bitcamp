...
-- 부모테이블 데이터 삭제 - 다른 테이블에서 참조하고 있으면 삭제 불가
-- 방법 1 : 자녀테이블 참조키 모두 삭제 또는 비활성화
DELETE FROM DEPT WHERE ID = 10; -- 참조하는 데이터들 떄문에 삭제 실행 안됨
--ORA-02292: integrity constraint (MADANG.EMP02_DEPTNO_FK) violated - child record found
ALTER TABLE EMP01 DISABLE CONSTRAINT SYS_C007035;
...
-- 방법2 : 서브테이블에 있는 FK 설정을 모두 삭제 후에 부모테이블 삭제 
---- FK 비활성화(DISABLE) 설정으로는 안됨 FK 삭제
ALTER TABLE C_TEST_SUB DROP CONSTRAINT C_TEST_SUB_FK;
DROP TABLE C_TEST_MAIN;

-- 방법3 : 부모테이블 삭제시 CASCADE CONSTRAINS 옵션 사용 
