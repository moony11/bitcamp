...
--------------------
-- 이름 김연어 -> 피겨스케이팅, 박세리 -> 골프 , 박시장 -> 축구선수, 나머지 -> 운동선수
SELECT NAME,
       CASE NAME
...
-- 20000 보다 크면 '비싸다', 30000보다 크면 '너무 비싸다'
SELECT * FROM BOOK;
SELECT B.*,
       CASE WHEN PRICE < 10000 THEN '싸다' ELSE
            CASE WHEN PRICE <= 20000 THEN '적당하다' ELSE
                 CASE WHEN PRICE <= 30000 THEN '비싸다' ELSE
                      CASE WHEN PRICE > 30000 THEN '너무 비싸다'
                      END
                 END 
            END
       END AS "가격평가"
