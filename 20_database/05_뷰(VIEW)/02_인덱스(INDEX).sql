...
alter index "MADANG"."IX_BOOK" unusable; -- 인덱스 사용불가 설정
SELECT * FROM BOOK WHERE PUBLISHER = '대한미디어'; -- 인덱스 적용
SELECT * FROM BOOK WHERE PRICE < 10000; -- 인덱스 적용 안함 

SELECT PUBLISHER, PRICE FROM BOOK ORDER BY PUBLISHER, PRICE; -- 위의 인덱스 이런형태로 만들어지는것 
-- WHERE 절에 PUBLISHER, PRICE 컬럼 동시사용시 IX_BOOK2 인덱스 적용 여부
SELECT * FROM BOOK WHERE PUBLISHER = '굿스포츠' AND PRICE = 7000;
SELECT * FROM BOOK WHERE PRICE = 7000 AND PUBLISHER = '굿스포츠'; -- 순서 상관 없음 두개가 사용됐으면 다 적용 
SELECT * FROM BOOK WHERE PUBLISHER = '굿스포츠' AND PRICE >= 7000;
